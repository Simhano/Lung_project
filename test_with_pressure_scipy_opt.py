...
original_positions   = mesh.points
...
mesh_points_onp = onp.array(mesh.points)  # Convert to standard NumPy array for optimization
...
initial_base_pressure = 2.0  # Or any reasonable initial guess
pressure_difference = 1.0  # Known pressure difference

def loss_function(params):
    global iii, total_loss_arr, current_mesh_dummy

    pressure_difference = 1
    base_internal_pressure = params[0]
    non_fixed_nodes_flat = params[1:]
#     # Reshape the flat array back to original node positions
    
    original_nodes = current_mesh_dummy.points.copy()
...
#     # Create a mesh with the current estimate of the original positions
    ...
    current_mesh.points = original_nodes
...
#     # Set up and solve the forward problem for pressure = 2
    problem_2 = HyperElasticity(
        ...
        dirichlet_bc_info=dirichlet_bc_info,
        location_fns=location_fns,
        internal_pressure=base_internal_pressure
    )
    sol_list_2 = solver(problem_2, solver_options={'petsc_solver': {}})
...
#     # Set up and solve the forward problem for pressure = 3
    problem_3 = HyperElasticity(
        ...
        dirichlet_bc_info=dirichlet_bc_info,
        location_fns=location_fns,
        internal_pressure=base_internal_pressure + pressure_difference
    )
    sol_list_3 = solver(problem_3, solver_options={'petsc_solver': {}})
    u_sol_3 = sol_list_3[0]
    ...
    # pressure_regularization = pressure_regularization_weight * (base_internal_pressure - expected_pressure) ** 2
    ...
    iii = iii + 1
    print('Iter')
    print(len(non_fixed_nodes_flat))
...
initial_base_pressure = 2
initial_nodes_flat = mesh.points[non_fixed_nodes].flatten()           #observed_positions_2[non_fixed_nodes].flatten()

initial_base_pressure = 2.4
initial_nodes_flat = observed_positions_2[non_fixed_nodes].flatten() *1.02

initial_params = np.hstack(([initial_base_pressure], initial_nodes_flat))
print(initial_nodes_flat.size)
print(initial_params.size)
...
def optimize_parameters():
    ...
    optimized_base_pressure = result.x[0]
    optimized_positions_flat = result.x[1:].reshape((-1, 3))
    return optimized_base_pressure, optimized_positions_flat


# Run the optimization
optimized_base_pressure, optimized_original_nodes_flat = optimize_parameters()

# Update mesh points with optimized positions
optimized_original_nodes = mesh_points_onp.copy()
optimized_original_nodes[non_fixed_nodes] = optimized_original_nodes_flat
