...
method_indicator = "gradually_increase_Known_Undeform_mass_with_adam_hmax_20_nu_48_location_51_" #
...
print(u_sol_2)
part_indicator = '_apply_negative_g_to_real_data.vtk'
...
#Mesh Update: From real MRI data to negative g applied geom (It is our pseudo-undeformed-configuraion)
mesh.points = onp.array(mesh.points + u_sol_2)

point_apply_negative_g_to_real_data = mesh.points
...
u_sol_2 = sol_list_2[0]
print(u_sol_2)
part_indicator = '_from_negative_g_to_postive_g_Intial_guess_data.vtk'
vtk_path_2 = os.path.join(data_dir, 'vtk', f"{method_indicator}{case_indicator}{part_indicator}")
os.makedirs(os.path.dirname(vtk_path_2), exist_ok=True)
save_sol(problem_2.fes[0], u_sol_2, vtk_path_2)

#Mesh Update: From negative g to possitive g
mesh.points = onp.array(mesh.points + u_sol_2)

point_apply_possitive_g_to_negative_g = mesh.points


observed_positions_2 = mesh.points
undeformed_coord = point_apply_negative_g_to_real_data #peudo undeformed
...
params = np.array(mesh.points) * 0 # 0.0001
tol = 1e-8
...
print("##############################################################")
print("##############################################################")
print("MRI data vs positive g")
print(np.sum((((MRI_mesh_points) - point_apply_possitive_g_to_negative_g ))**2)/np.sum((point_apply_possitive_g_to_negative_g )**2))
print("##############################################################")
print("##############################################################")
print("##############################################################")
print("##############################################################")
...
current_mesh = mesh
relax_flag = 0
# original_density_arr = [50 ,100, 150, 200, 250, 300, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 942.8238474]
original_density_arr = onp.arange(100, 943, 1)
